"""Voyager memory adapter.

Wraps G-Memory's Voyager implementation (vendor/GMemory/mas/)
into EquiMem's EmbeddingMemory interface.

Voyager stores skill descriptions and code as dense vectors.
Originally designed for Minecraft; here used as a general
embedding-based memory for multi-agent debate.
"""
...
class VoyagerAdapter(EmbeddingMemory):
    """Wrap G-Memory's Voyager into EmbeddingMemory interface."""
    ...
    def __init__(self, llms_type: list[str], embed_model: str = "all-MiniLM-L6-v2"):
        ...
        # Try loading vendor Voyager for its internal logic
        self._vendor = None
        try:
            from mas.memory_voyager import MemoryVoyager
            self._vendor = MemoryVoyager(llms_type)
        except ImportError:
            pass
    ...
    def search(self, query: str, top_k: int = 5):
        ...
        # Apply trust discount: scale similarities by sqrt(rho)
